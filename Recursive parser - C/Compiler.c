...
static void assign()
{
    ...
    next_token();
    if(token != '='){
        ERROR("Expected Assign %c \n", token);
        exit(EXIT_FAILURE);
    }
    next_token();
    reg = expr();
    CodeGen(STORE, ch, reg, EMPTY_FIELD);
